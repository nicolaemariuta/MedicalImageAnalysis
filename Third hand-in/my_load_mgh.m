function [vol, pixdim] = my_load_mgh(fname,slices,frames,headeronly)

% An adaption of load_mgh from the FreeSurfer Matlab toolbox that also
% works on windows (assumes 7zip installed)
...
if (strcmpi(fname((strlen(fname)-3):strlen(fname)), '.MGZ') | ...
  ...
  elseif(strcmp(computer, 'PCWIN64'))
    system(sprintf('"C:/Program Files/7-Zip/7z.exe" e %s', fname));  
    [pathstr, name, ext] = fileparts(fname);
    system(sprintf('move %s %s', name, new_fname));
  else
...
pixdim = [sqrt(sum(M(:,1).^2)); ... % Col
    sqrt(sum(M(:,2).^2)); ... % Row
    sqrt(sum(M(:,3).^2))]; % Slice
